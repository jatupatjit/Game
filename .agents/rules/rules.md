...
- **Context Auto-Load**: Always reference [`.agents/ctx.md`](../ctx.md) (or `@ctx.md`) for comprehensive project architecture, active component details, rigging structure, and bug history.
...
- At the conclusion of each milestone or major bug fix, automatically summarize the latest changes, resolved issues, and next tasks directly into [`.agents/ctx.md`](../ctx.md) and keep it mirrored to `ctx.md`. Keep it concise under 300 words using bullet points.
