...
## 🎮 Project-Specific Core Rules (Unity 6 + NGO Co-op Physics)
1. **Engine & Networking Framework**: The project strictly targets **Unity 6 (URP)** and **Netcode for GameObjects (NGO 2.x)**.
2. **Network Synchronization Class**: Any gameplay or online data-synchronizing script must inherit from **NetworkBehaviour** (not plain MonoBehaviour).
3. **Physics Synchronization**: All physics forces and Rigidbody operations (forces, torque, velocity, position modification) must strictly occur in **FixedUpdate** to ensure deterministic physics replication across network clients.
